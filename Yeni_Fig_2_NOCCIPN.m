...
%%%%%%%%%%%%%%%%%%%%%%%%NO_PN&CCI Hesaplanýyor%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Y_SR_NoPNCCI = (PS.*Pss.*hsr_square)./(1);% Equation (3)                                     
Y_RD_NoPNCCI  = (PR.*Pss.*hrd_square)./(1); %Equation (4)                                           

Y_RE_NoPNCCI  = (PR.*hre_square);% Equation (7)
Y_SE_NoPNCCI  = (PS.*hse_square);% Equation (6)

C_SR_NoPNCCI  =  (1+Y_SR_NoPNCCI); % Equation (8)
C_SE_NoPNCCI  =  (1+Y_SE_NoPNCCI); % Equation (8)

C_RD_NoPNCCI  = (1+Y_RD_NoPNCCI); % Equation (8)
C_RE_NoPNCCI  = (1+Y_RE_NoPNCCI); % Equation (8)

C_FirstHop_NoPNCCI   = C_SR_NoPNCCI./C_SE_NoPNCCI;% Equation (8) - Denklem 8'in ilk terimi
C_SecondHop_NoPNCCI  = C_RD_NoPNCCI./C_RE_NoPNCCI;% Equation (8)- Denklem 8'in ikinci terimi
C_e2e_NoPNCCI = min(C_FirstHop_NoPNCCI,C_SecondHop_NoPNCCI);% Equation (8)


SOP(jj) = length(find(C_e2e<p))./N % Equation (9)
SOP_NoCCI(jj) = length(find(C_e2e_NoCCI<p))./N % Equation (9)
SOP_NoPNCCI(jj) = length(find(C_e2e_NoPNCCI<p))./N % Equation (9)
...
semilogy(Ps_vec, (SOP_NoPNCCI),'Linewidth',1,'Color',[0.0,1.0,0.0],'LineStyle','-');
...
Teo_CCI=[0.968265, 0.913627, 0.824138, 0.702919, 0.563991, 0.426726, 0.306928,0.212093, 0.14228, 0.0934767, 0.0605498, 0.0388553, 0.0247821,0.0157442, 0.00997731, 0.00631285];
...
ylabel('SOP')
legend('Çalýþma','[22]','Eþ Kanal ve Birincil Að Giriþimi Yok','Analitik');
grid on
